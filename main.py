...
STYLES = {
    "drama": {"speed": 1.0, "color": 1.3, "fade": 0.3},
    "fun":   {"speed": 1.5, "color": 1.0, "fade": 0.1},
    "hype":  {"speed": 1.3, "color": 1.5, "fade": 0.1},
}
...
def process(path, style, clip_dur):
    ...
    # Prend le début (plus rapide que chercher le meilleur moment)
...
async def montage(req: Req):
    if not req.video_urls: raise HTTPException(400, "Pas de vidéos")
    job = str(uuid.uuid4())[:8]
    tmp = Path(tempfile.mkdtemp())
    try:
        clips = []
        for i, url in enumerate(req.video_urls[:3]):
            dest = tmp / f"v{i}.mp4"
            await dl(url, dest)
            c = process(dest, req.style, req.clip_duration)
            clips.append(c)
